...
const createUser = async (req, res) => {
  ...
  const checkEmployeeUserResult = await pool.query(checkEmployeeUserQuery, [
    employee_id,
  ]);
  ...
  try {
    ...
    const untitledResult = await connection.query(
      insertUntitledQuery,
...
const updateUser = async (req, res) => {
  ...
  try {
    ...
    const existingEmployeeQuery =
      "SELECT * FROM users WHERE employee_id = ? AND user_id !=? ";
    const existingEmployeeResult = await connection.query(
...
const onChangePassword = async (req, res) => {
  ...
  await Promise.all([
    ...
    body("new_password")
      .notEmpty()
      .withMessage("New password is requierd.")
      .isLength({ min: 8 })
  ...
  try {
    ...
    const [checkUserResult] = await connection.query(checkUserQuery, [email.toLowerCase()]);
...
const verifyOtp = async (req, res) => {
  ...
  try {
    ...
    const verifyOtpQuery = `
      ...
      `;
    const verifyOtpResult = await connection.query(verifyOtpQuery, [
...
const forgotPassword = async (req, res) => {
  await Promise.all([
    ...
    body("confirmPassword")
      .notEmpty()
      .withMessage("Confirm password is requierd.")
      .isLength({ min: 8 })
      .withMessage("Password must be at least 8 characters long.")
      .matches(/[0-9]/)
      .withMessage("Password must contain at least one number.")
